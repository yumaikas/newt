...
(defn- print-help [] 
    ...
    Commands/flags
      ...
      -x: Actually writes out files, rather than listing operations
...
(defn- ensure-project-license [opts] 
    (when (opts :newt/suppress-license) 
        (break))
    (if-let [ 
        project-license (or 
            (opts :newt/project-name)
        ...
        (put opts :newt/project-license project-license)
        (err/signal 
            :newt/missing-project-info 
...
(defn create-project [opts] 
    (def project.janet (template/project.janet opts))
    ...
    # Make test folder
    # Spit test file
    # If requested, git init (not metioned elsewhere)
...
(defn main [exename & args] 
    ...
    (err/try* (do
        ...
        [:newt/missing-project-info message] (do 
            (eprint message)
            (os/exit 1))
    )
